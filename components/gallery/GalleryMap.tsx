...
export function GalleryMap({ onCharacterClick, focusCharacterId, onLogoClick }: GalleryMapProps) {
  ...
  useEffect(() => {
    const unsub = useRevealedStore.subscribe((state) => {
      characterContainerCache.forEach((wrapper, cacheKey) => {
        const charId = cacheKey;
        if (state.allRevealed || state.revealedCharacters.includes(charId)) {
          const charContainer = wrapper.children[0] as Container | undefined;
          if (charContainer && charContainer.children.length > 1) {
            const avatarSprite = charContainer.children[0] as Sprite | undefined;
            const giftContainer = charContainer.children[1] as Container | undefined;
            if (giftContainer && giftContainer.renderable) {
              giftContainer.renderable = false;
              giftContainer.alpha = 0;
              if (avatarSprite) avatarSprite.alpha = 1;
  ...
  const createCharacterContainer = useCallback(async (character: Character, index: number): Promise<Container> => {
    ...
    const giftContainer = new Container();
    ...
    wrapper.on('pointerdown', (e) => {
      if (!isPinchingRef.current) {
        const isRevealed = useRevealedStore.getState().isRevealed(character.id);

        if (!isRevealed) {
          useRevealedStore.getState().revealCharacter(character.id);

          const particlesContainer = new Container();
          wrapper.addChild(particlesContainer);

          const colors = [0xf97316, 0xf59e0b, 0xfbbf24, 0xffffff];
          const particles: { sprite: Graphics, vx: number, vy: number, life: number, maxLife: number, rotationSpeed: number }[] = [];
          ...
          const animateReveal = () => {
            ...
            if (alpha > 0) {
              giftContainer.alpha = alpha;
              if (charContainer.children.length > 0) {
                const avatarSprite = charContainer.children[0];
                if (avatarSprite) avatarSprite.alpha = 1 - alpha;
              }
              const s = 0.95 + ((1 - alpha) * 0.1);
              charContainer.scale.set(Math.min(s, 1.05));
              ...
              let bounceScale = 1.05;
              const animateBounce = () => {
                bounceScale -= 0.01;
                if (bounceScale > 1) {
                  charContainer.scale.set(bounceScale);
              ...
              const clearParticles = () => {
                ...
                if (stillAlive) {
                  requestAnimationFrame(clearParticles);
                } else {
                  wrapper.removeChild(particlesContainer);
